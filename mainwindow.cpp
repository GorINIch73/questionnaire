...
void MainWindow::on_actionNewBase_triggered()
{
    ...
    QString newBase =  QFileDialog::getSaveFileName(this,tr("Create new base"),"./",tr("Data base Fules (*.qustnr)"));
     ...
     // открываем базу
         ...
         str = "INSERT INTO setings(option_name) values ('Наименование анкеты'),('Примечание'),('ID вопроса СМП'),('ID ответа СМП'),('ID вопроса удовлетворенности СМП'),('ID ответа удовлетворенности СМП пусто');";
         if (!a_query.exec(str))
             qDebug() << "таблица настроек: " << a_query.lastError().text();
